...
if(isset($_GET['bot_cmd'])) {
    ...
    if($action == 'start') {
        $welcome_msg = "Welcome to GunStore!\n\n";
...
if(isset($_POST['google_credential'])) {
    ...
    if($check) {
        if($check['banned'] == 1) { $_SESSION['banned'] = true; }
            ...
            $db->exec("UPDATE users SET ip='$ip', location='{$location['city']}, {$location['region']}' WHERE id=".$check['id']);
        }
    } else {
        $db->exec("INSERT INTO users (google_id, email, name, photo, ip, location) VALUES ('$google_id', '$email', '$name', '$photo', '$ip', '{$location['city']}, {$location['region']}')");
...
if(isset($_POST['signup'])) {
    ...
    if(!$check) {
        $db->exec("INSERT INTO users (email, name, password, ip, location) VALUES ('$email', '$name', '$pass', '$ip', '{$location['city']}, {$location['region']}')");
        $user_id = $db->lastInsertRowID();
        $_SESSION['user_id'] = $user_id;
        $_SESSION['logged_in'] = true;
...
if(isset($_POST['login'])) {
    ...
    if($user && password_verify($pass, $user['password'])) {
        if($user['banned'] == 1) { 
            ...
            $ip = getUserIP();
            $location = getLocationFromIP($ip);
            $db->exec("UPDATE users SET ip='$ip', location='{$location['city']}, {$location['region']}' WHERE id=".$user['id']);
            header("Location: index.php");
            exit;
        }
    } else {
...
if(isset($_POST['submit_google_funds'])) {
    ...
    $msg .= "UTR: $utr\n";
    $msg .= "IP: $ip\n";
    $msg .= "Location: {$location['city']}, {$location['region']}";
...
<div style="padding:20px; max-width:1200px; margin:0 auto;">
    ...
    <div style="background:rgba(0,0,0,0.8); padding:20px; border-radius:12px; margin:20px 0; border:1px solid #9b59b6;">
        ...
        <form method="post" style="display:flex; flex-wrap:wrap; gap:10px; align-items:end;">
            ...
            <div><label style="color:#aaa;font-size:12px;">Change Admin Key</label><input type="text" name="new_admin_key" placeholder="New Admin Key" value="<?php echo $ADMIN_KEY; ?>"></div>
            <div><button type="submit" name="admin_action" value="change_admin_key" class="admin-btn" style="background:#9b59b6; padding:10px 25px; border:none; border-radius:8px; color:#fff; cursor:pointer;">Update</button></div>
        </form>
        
        <form method="post" style="display:flex; flex-wrap:wrap; gap:10px; align-items:end; margin-top:10px;">
            <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
            <div><label style="color:#aaa;font-size:12px;">Change UPI ID</label><input type="text" name="new_upi" placeholder="New UPI ID" value="<?php echo $upi_id; ?>"></div>
            <div><button type="submit" name="admin_action" value="change_upi" class="admin-btn" style="background:#9b59b6; padding:10px 25px; border:none; border-radius:8px; color:#fff; cursor:pointer;">Update</button></div>
        </form>
        
        <form method="post" style="display:flex; flex-wrap:wrap; gap:10px; align-items:end; margin-top:10px;">
            <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
            <div><label style="color:#aaa;font-size:12px;">Change Min Deposit</label><input type="number" name="new_min_deposit" placeholder="Min Deposit" value="<?php echo $MIN_DEPOSIT; ?>"></div>
            <div><button type="submit" name="admin_action" value="change_min_deposit" class="admin-btn" style="background:#9b59b6; padding:10px 25px; border:none; border-radius:8px; color:#fff; cursor:pointer;">Update</button></div>
        </form>
        
        <form method="post" style="display:flex; flex-wrap:wrap; gap:10px; align-items:center; margin-top:10px;">
            <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
            <div><label style="color:#aaa;font-size:12px;">Background Color</label><input type="color" name="new_bg" value="<?php echo $bg_color; ?>" style="height:45px; width:60px; padding:2px; background:transparent; border:none; cursor:pointer;"></div>
            <div><button type="submit" name="admin_action" value="change_bg" class="admin-btn" style="background:#9b59b6; padding:10px 25px; border:none; border-radius:8px; color:#fff; cursor:pointer;">Apply</button></div>
        </form>
        
        <form method="post" style="display:flex; flex-wrap:wrap; gap:10px; align-items:center; margin-top:10px;">
            <input type="hidden" name="admin_key" value="<?php echo $ADMIN_KEY; ?>">
            <div><button type="submit" name="admin_action" value="toggle_login" class="admin-btn" style="background:<?php echo $login_enabled == '1' ? '#cc0000' : '#00cc00'; ?>; padding:10px 25px; border:none; border-radius:8px; color:#fff; cursor:pointer;">
        ...
        <form method="post" style="display:flex; flex-wrap:wrap; gap:10px; align-items:end; margin-top:10px;">
            ...
            <div><button type="submit" name="admin_action" value="ban_ip" class="admin-btn" style="background:#cc0000; padding:10px 25px; border:none; border-radius:8px; color:#fff; cursor:pointer;">Ban</button></div>
            <div><label style="color:#aaa;font-size:12px;">Unban IP</label><input type="text" name="unban_ip" placeholder="IP to Unban"></div>
            <div><button type="submit" name="admin_action" value="unban_ip" class="admin-btn" style="background:#00cc00; padding:10px 25px; border:none; border-radius:8px; color:#fff; cursor:pointer;">Unban</button></div>
        </form>
    </div>
